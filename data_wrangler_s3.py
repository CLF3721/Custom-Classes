...
# # file_key = 'data/clean/partners.csv'
# bucket_name = 'dynamic-bidding'
# prefix = 'data/clean/'

# wrangle = s3DataWrangler(bucket_name, prefix, s3)
# dict_of_dfs = wrangle()
...
# def s3_obj_to_dict_of_dataframes(bucket_name: str, prefix: str) -> dict:
#     '''
#     Converts an object from an S3 bucket to a Pandas DataFrame.
#     '''
#     object_contents = s3.list_objects_v2(Bucket=bucket_name, Prefix=prefix).get('Contents', [])
#     list_of_file_keys = []
#     for obj in object_contents:
#         list_of_file_keys.append(obj['Key'])
#     list_of_file_keys = [e for e in list_of_file_keys if '.' in e]

#     dfs = {}
#     for file_key in sorted(list_of_file_keys):
...
#                     )
#             elif file_key.endswith('.json'):
#                 df_raw = pd.read_json(BytesIO(body))
#             elif file_key.endswith('.parquet'):
#                 df_raw = pd.read_parquet(BytesIO(body))
#             else:
#                 raise ValueError("Unsupported file type")
...
# dfs = s3_to_dataframes(bucket_name, prefix)


# def get_filenames(dfs: dict) -> list:
#     '''
#     Reads all files in the specified directory, 
#     stems the filenames, then returns a list of
#     the stemmed filenames which exist in the directory.
#     :return: A list of the filename stems in entire directory
#     '''
#     filenames = [(file.rsplit('/')[2]).lower() for file in list(dfs.keys())]
#     df_names = [(file.rsplit('.', 1)[0]).lower() for file in filenames]
#     zipped_filenames = list(zip(df_names, filenames))
#     return zipped_filenames
# get_filenames(dfs)
